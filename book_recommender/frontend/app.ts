...
const prepareButton = document.getElementById("prepare-btn") as HTMLButtonElement;
const prepareStatus = document.getElementById("prepare-status") as HTMLParagraphElement;
const recommendForm = document.getElementById("recommend-form") as HTMLFormElement;
const recommendStatus = document.getElementById("recommend-status") as HTMLParagraphElement;
const targetTitleInput = document.getElementById("target-title") as HTMLInputElement;
const titleSuggestions = document.getElementById("title-suggestions") as HTMLDataListElement;
const authorSubstringInput = document.getElementById("author-substring") as HTMLInputElement;
const thresholdInput = document.getElementById("threshold") as HTMLInputElement;
const topNInput = document.getElementById("top-n") as HTMLInputElement;
const tableBody = document.querySelector<HTMLTableSectionElement>("#results-table tbody");
...
prepareButton.addEventListener("click", async () => {
  ...
  try {
    ...
    prepareStatus.textContent = `Error: ${(error as Error).message}`;
  } finally {
...
recommendForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const items: RecommendationItem[] = payload.items;
    renderRows(items);
    const matchedTitle = payload.matched_title as string;
    recommendStatus.textContent = `Done. Received ${items.length} recommendations. Matched title: "${matchedTitle}".`;
  } catch (error) {
    recommendStatus.textContent = `Error: ${(error as Error).message}`;
  }
});
...
function renderRows(items: RecommendationItem[]): void {
  if (!tableBody) return;
...
async function fetchTitleSuggestions(query: string): Promise<void> {
  ...
  renderTitleSuggestions(payload.items as SuggestionItem[]);
}

function renderTitleSuggestions(items: SuggestionItem[]): void {
  titleSuggestions.innerHTML = "";
  for (const item of items) {
